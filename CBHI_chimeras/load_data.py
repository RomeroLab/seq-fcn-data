...
block_alignment, column_names = read_alignment('CBHI_block_alignment.aln')

# load contacts 
contacts = pickle.load(open('CBHI_contacts.pkl','rb'))

# load binary function data 
names,function_data = read_data('CBHI_function.data')
AAseqs_fcn = [chimera2sequence(block_alignment,f[0]) for f in function_data]

# load thermostability data
names,thermo_data = read_data('CBHI_thermostability.data')
